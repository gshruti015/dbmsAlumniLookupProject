...
try {
    ...
    if (empty($_POST['firstname']) AND !empty($_POST['comapnyname']) AND empty($_POST['Visa']))
    {
        $stmt = $conn->prepare("SELECT * from SMITH_UC where COMNAME = '$company'");
    }  
    if (empty($_POST['firstname']) AND !empty($_POST['comapnyname']) AND !empty($_POST['Visa']))
    {
    ...
    echo "<tr style='border-collapse:collapse;border: 1pt solid black;padding-top: 12px;padding-bottom: 12px;text-align: center;background-color: #b6b6ff; color: white;'><th>First Name</th><th>Last Name</th><th>Contact Number</th><th>Visa Status</th><th>Job Title</th><th>Network</th><th>Industry</th><th>Company</th></tr>";
